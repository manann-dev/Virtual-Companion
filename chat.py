...
from html_generator import chat_html_wrapper, make_thumbnail
...
import gradio as gr
from utils import delete_file, get_available_characters, save_file
...
def generate_chat_prompt(user_input, state, **kwargs):
    ...
    print("Current state:", state)  # Debug: Check what state looks like
    if 'history' not in state or not isinstance(state['history'], dict):
        raise ValueError("History is missing or not a dictionary")

    history = state['history'].get('internal', [])

    # history = kwargs.get('history', state['history'])['internal']
    # history = kwargs.get('history', {}).get('internal', state.get('history', {}).get('internal'))
    # history = kwargs.get('history',state.get('history',[]))

    print("Received user input:", user_input)
    print("Impersonate:", impersonate, "_continue:", _continue, "Also return rows:", also_return_rows)
    print("History at start:", history)
    ...
    print("Chat template string:", chat_template_str)
    ...
    print("Messages after history processing:", messages)
...
# def generate_chat_reply(text, state, regenerate=False, _continue=False, loading_message=True, for_ui=False, chat_template_str=None):
#     # Use chat_template_str as needed within this function
#     history = state['history']
#     if regenerate or _continue:
#         text = ''
#         if (len(history['visible']) == 1 and not history['visible'][0][0]) or len(history['internal']) == 0:
...
# def generate_pfp_cache(character):
    ...
    for extension in ['png', 'jpg', 'jpeg']:
        path = Path(f"characters/{character}.{extension}")
        if path.exists():
...
def generate_pfp_cache(character):
    ...
    for extension in ['png', 'jpg', 'jpeg']:
        ...
        if path.exists():
            ...
            return str(path)  # Return the file path as a string
...
def load_character(character, name1, name2):
    ...
    return name1, name2, encoded_string, greeting, context
...
def delete_character(name, instruct=False):
    ...
    logger.info(f'Deleted {name}.' if not instruct else f'Character {name} deleted.')
