    ...
    for sampler in all_samplers:
        if loader == 'All' or sampler in loaders_samplers[loader]:
            if sampler.startswith('dynatemp'):
                output.append({'sampler': sampler, 'visible': dynamic_temperature})
            else:
                output.append({'sampler': sampler, 'visible': True})
        else:
            output.append({'sampler': sampler, 'visible': False})
...
def make_loader_params_visible(loader):
    ...
    visibility_status = {param: True if param in params else False for param in all_params}

    return json.dumps(visibility_status)