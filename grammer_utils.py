...
import shared
from utils import get_available_grammars
...
#     print("Initializing grammar for:", grammar_string)
#     return IncrementalGrammarConstraint(grammar_string.strip(), start_rule_name="root", tokenizer=shared.tokenizer)

@lru_cache(maxsize=5)
def initialize_grammar(grammar_file_name):
    available_grammars = get_available_grammars()
    print(f"available_grammars: {available_grammars}")
    # if grammar_file_name not in available_grammars:
    #     print(f"Error: Grammar file '{grammar_file_name}' not found.")
    #     return None

    if grammar_file_name == 'None':
        return None

    grammar_file = Path('grammars') / f"{grammar_file_name}.gbnf"
    with open(grammar_file, 'r') as f:
        grammar_str = f.read()
    print(f"Initializing grammar from file: {grammar_file}")
    return IncrementalGrammarConstraint(grammar_str.strip(), start_rule_name="root", tokenizer=shared.tokenizer)