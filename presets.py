...
import shared
from loaders import loaders_samplers
    ...
    if name not in ['None', None, '']:
        ...
        if path.exists():
            ...
            for k, v in preset.items():
                if k == 'sampler_priority' and isinstance(v, str):
                    generate_params[k] = [x.strip() for x in v.split('\n') if x.strip()]
                else:
                    generate_params[k] = v
        else:
            logger.error(f"The preset \"{name}\" does not exist under \"{path}\". Using default parameters.")
    if verbose:
