...
#     config_path = f'models/facebook_opt-1.3b/tokenizer_config.json'  # Adjust path format if necessary
#     try:
#         with open(config_path, 'r') as file:
#             config = json.load(file)
        
#         model_path_or_name = config["name_or_path"]
#         tokenizer = AutoTokenizer.from_pretrained(model_path_or_name)
#         print("Tokenizer loaded successfully.")
#         return tokenizer
#     except Exception as e:
#         print(f"Failed to load tokenizer from {model_path_or_name}: {e}")
#         raise
...
def llamacpp_HF_loader(model_name):
    from llamacpp_hf import LlamacppHF
...
def unload_model(model_name=shared.model_name):
